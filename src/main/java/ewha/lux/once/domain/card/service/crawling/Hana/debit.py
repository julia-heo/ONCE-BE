...
'''
    카드 정보 조회
    hana_debitCardInfos.csv : card_name, card_url, card_img
'''
url = "https://www.hanacard.co.kr/OPI31000000D.web?schID=pcd&mID=OPI31000005P&CT_ID=241704050328506"
...
df.to_csv("src/main/java/ewha/lux/once/domain/card/service/crawling/Hana/hana_debitCardInfos.csv", encoding = "utf-8-sig")

'''
    전체 카드 혜택 크롤링
    debit_benefit.csv : card_company_id, name, img_url, benefits, created_at, type
'''
card_infos = pd.read_csv('src/main/java/ewha/lux/once/domain/card/service/crawling/Hana/hana_debitCardInfos.csv')
...
for i, url in enumerate(card_urls):
    ...
    if len(tab_list) == 0: # 탭이 하나도 없는 경우
        ...
        card_list = card_view_detail.find('ul', {'class': 'card_li'})
        if card_list is not None:
            info_list = card_list.find_all('li', {'class': 'list'})
        for info in info_list:
            tit = info.find('dt', {'class': 'tit'})
            if tit is not None:
                tit_text = str(tit.get_text(separator=' ', strip=True)) 
            benefit += f'[{tit}]'
                
            info_txt = info.find('dd', {'class': 'txt'})
            if info_txt is not None:
                info_txt_list = info_txt.find_all('li', {'class': 'blt1'})
...
df.to_csv("src/main/java/ewha/lux/once/domain/card/service/crawling/Hana/debit_benefit.csv", encoding = "utf-8-sig", index=False)