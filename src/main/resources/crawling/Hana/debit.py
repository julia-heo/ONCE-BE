...
print("======= [하나] 체크 카드 정보 크롤링 =======", flush=True)
...
df.to_csv("/crawling/Hana/hana_debitCardInfos.csv", encoding = "utf-8-sig")

'''
    전체 카드 혜택 크롤링
    debit_benefit.csv : card_company_id, name, img_url, benefits, created_at, type
'''
card_infos = pd.read_csv('/crawling/Hana/hana_debitCardInfos.csv')
...
print("======= [하나] 전체 카드 혜택 정보 크롤링 =======", flush=True)
...
df.to_csv("/crawling/Hana/debit_benefit.csv", encoding = "utf-8-sig", index=False)